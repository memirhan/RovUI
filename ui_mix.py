...
class ROVConsole(QtWidgets.QMainWindow):
    def __init__(self, camera_index=0):
        ...
        # Seri port okuma için timer
        self.serialTimer = QtCore.QTimer(self)
        self.serialTimer.timeout.connect(self.read_serial)
        self.serialTimer.start(20)  # 20ms (50Hz)
        ...
        # Video (kodu kısalttım)
        self.video = VideoWidget()
        top.addWidget(self.video, 2)
        ...
        self.joystickXY = JoystickWidget()         # sol joystick
        self.joystickY  = JoystickWidget(lock_x=True)  # sağ joystick
    ...
    def read_serial(self):
        if self.ser and self.ser.in_waiting > 0:
            try:
                ...
                # Örn: "500,600" → sağ joystick için (X sabit, Y değişiyor)
                if len(su) >= 2:
                    x = int(su[0])
                    y = int(su[1])

                    # SAĞ joystick’i güncelle
                    self.joystickY.update_position(x, y)
...
# Sol joystick (normal X-Y hareket)
        self.joystickSag = JoystickWidget()

        # Sağ joystick (sadece Y ekseni hareketli)
        self.joystickSol = JoystickWidget(lock_x=True)

        joyLayout.addWidget(self.joystickSag)
        joyLayout.addWidget(self.joystickSol)
        cl.addLayout(joyLayout)

        # Test amaçlı rastgele hareket (gerçek veride kaldır)
        def test_move():
            import random
            self.joystickSag.update_position(
                random.randint(400, 600),  # X
                random.randint(400, 600)   # Y
            )
            self.joystickSol.update_position(
                512,                       # X sabit
                random.randint(400, 600)   # Y
            )
